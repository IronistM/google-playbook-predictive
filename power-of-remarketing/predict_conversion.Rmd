...
The following variables were excluded to keep the number of columns manageable:
...
## Removing low volumn columns

To reduce data size and help with modeling functions later, remove columns that will have little impact.  For example, a page that has only been visited once by one observation.
...
To check the results, the model should get a similar `transacted` value in the validation set when applied to the test and validation data sets.
...
In the example below, we are comparing actual (All Users) vs predicted (Model Segment) user transactions (e.g. - "Goal 1 - Purchase Completed" goal completions). We can see that our model predicts 580 users will make transactions vs the actual 600 users transacting - 96% of users!
